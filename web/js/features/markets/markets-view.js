...
function formatProbabilityDisplay(probability) {
  return `${Math.round(typeof probability === 'number' ? probability : 0)}¢`;
}

function formatCloseDateDisplay(closeDate) {
  return closeDate && closeDate !== 'TBD' ? closeDate : '--';
}

function formatPercentDisplay(probability) {
  return `${Math.round(typeof probability === 'number' ? probability : 0)}%`;
}
...
/**
 * Generate deterministic probability history for an outcome
 * If there is no market history yet, this stays flat.
...
function generateProbabilityHistory(cacheKey, currentProb, points = 30) {
  ...
  const safeProb = typeof currentProb === 'number' ? Math.max(0, Math.min(100, currentProb)) : 0;
  const history = Array.from({ length: pointCount }, () => safeProb);

  // Ensure the last point (rightmost on graph) equals the current probability
  history[history.length - 1] = safeProb;
...
function generateMarketHistories(marketId, outcomes, points = 30) {
  ...
  if (outcomes.length === 2) {
    ...
    complementHistory[complementHistory.length - 1] =
      typeof outcomes[1].probability === 'number' ? outcomes[1].probability : complementHistory[complementHistory.length - 1];
  ...
  const legend = outcomes.map((outcome, idx) => {
    ...
    return `
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-medium text-slate-300">${formatProbabilityDisplay(outcome.probability)}</span>
      </div>
    `;
  });
  ...
  const legends = outcomes.slice(0, displayCount).map((outcome, idx) => {
    ...
    return `
      <div class="flex items-center gap-1">
        ...
        <span class="text-xs text-slate-300">${formatProbabilityDisplay(outcome.probability)}</span>
      </div>
    `;
  });
...
async function fetchCurrentEventMarkets() {
  try {
    const response = await fetch('/api/markets/current-events');
    if (response.ok) {
...
async function fetchMarketSuggestions() {
  try {
    const response = await fetch('/api/markets/suggestions');
    if (response.ok) {
...
async function handleWatchlistClick(marketId, marketTitle, category, buttonEl) {
  if (typeof toggleWatchlist === 'function') {
    ...
    // Update the button
    if (isNowWatchlisted) {
      buttonEl.innerHTML = window.SamsaIcons ? window.SamsaIcons.getStarHTML(true, 'w-5 h-5') : '★';
      ...
      buttonEl.innerHTML = window.SamsaIcons ? window.SamsaIcons.getStarHTML(false, 'w-5 h-5') : '☆';
...
function createTrendingSlide(market) {
        ...
        ${market.outcomes.slice(0, 2).map((outcome, idx) => `
          <button class="flex-1 relative overflow-hidden rounded-lg px-3 py-2 transition-all duration-200 border ${idx === 0 ? 'bg-green-500/10 border-green-500/50 hover:border-green-500 hover:bg-green-500/20' : 'bg-red-500/10 border-red-500/50 hover:border-red-500 hover:bg-red-500/20'} active:scale-95" onclick="event.stopPropagation(); openPredictionForm('${market.id}', '${outcome.id}')">
            <div class="flex flex-col gap-1">
              ...
              <span class="text-base font-semibold text-center ${idx === 0 ? 'text-green-400' : 'text-red-400'}">${formatProbabilityDisplay(outcome.probability)}</span>
            </div>
          </button>
        `).join('')}
      </div>
    ...
    tradingOptions = `
      ...
      return `
            <button class="w-[calc(50%-4px)] relative overflow-hidden rounded-lg px-3 py-2 transition-all duration-200 border ${colors.bg} ${colors.border} ${colors.hover} active:scale-[0.98] flex flex-col items-center justify-center" onclick="event.stopPropagation(); openPredictionForm('${market.id}', '${outcome.id}')">
              ...
              <span class="text-base font-semibold ${colors.text}">${formatProbabilityDisplay(outcome.probability)}</span>
            </button>
          `;
    }).join('')}
  ...
  const watchlistIcon = window.SamsaIcons ? window.SamsaIcons.getStarHTML(isInWatchlist, 'w-5 h-5') : (isInWatchlist ? '★' : '☆');
      ...
      <div class="flex items-start justify-between mb-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="text-right">
            ...
            <p class="text-sm text-white font-normal">${formatCloseDateDisplay(market.closeDate)}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-800/30 rounded-xl p-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-sm font-medium text-white">${formatProbabilityDisplay(market.outcomes[0]?.probability)}</span>
          </div>
        ...
        <div class="flex flex-col justify-between">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="bg-slate-800/30 rounded-lg p-3 text-center">
              ...
              <p class="text-sm font-semibold text-yellow-400">$${((market.volume || 0)).toLocaleString()}</p>
            </div>
            <div class="bg-slate-800/30 rounded-lg p-3 text-center">
              <p class="text-xs text-slate-500 mb-1">24h</p>
              <p class="text-sm font-semibold text-yellow-400">$${((market.volume24h || 0)).toLocaleString()}</p>
            </div>
            <div class="bg-slate-800/30 rounded-lg p-3 text-center">
              <p class="text-xs text-slate-500 mb-1">Traders</p>
              <p class="text-sm font-semibold text-yellow-400">${market.traders || 0}</p>
            </div>
          </div>
  ...
  if (market.sparkline && Array.isArray(market.sparkline.points) && market.sparkline.points.length > 0) {
    ...
    if (isBinary) {
      ...
      for (let i = 1; i < Math.min(outcomes.length, 4); i++) {
        histories.push(generateProbabilityHistory(`m-${market.id}-${i}`, outcomes[i]?.probability ?? 0, pts.length));
      }
    }
  } else {
...
function getMarketTypeLabel(market) {
  if (isBinaryMarket(market)) {
    const icon = window.SamsaIcons ? window.SamsaIcons.getIconByKey('balance', 'w-3 h-3', '#34d399') : '';
    return { text: 'Binary', icon, class: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' };
  }
  const icon = window.SamsaIcons ? window.SamsaIcons.getIconByKey('barchart', 'w-3 h-3', '#c084fc') : '';
      ...
      <div class="[grid-area:1/1] p-4 flex flex-col gap-2 z-10">
        <div class="flex items-start justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-green-400 font-medium flex items-center gap-1"><span class="w-2 h-2 bg-green-400 rounded-full animate-pulse inline-block"></span>Live</span>
          </div>
        </div>
...
function generateDetailHTML(market) {
  ...
  const closeDate = formatCloseDateDisplay(safeMarket.closeDate);
    ...
    <div class="max-w-7xl mx-auto">
      ...
      safeMarket.outcomes.map((outcome, idx) => {
        ...
        return `
                  <button id="outcomeBtn-${outcome.id}" onclick="showInlineTradingForm('${safeMarket.id}', '${outcome.id}')" 
                    ...
                    <span class="text-xl font-semibold ${isYes ? 'text-green-400' : 'text-red-400'}">${formatProbabilityDisplay(outcome.probability)}</span>
                  </button>
                `;
      }).join('')
      :
      safeMarket.outcomes.map((outcome, idx) => {
        ...
        return `
                  <button id="outcomeBtn-${outcome.id}" onclick="showInlineTradingForm('${safeMarket.id}', '${outcome.id}')" 
                    ...
                    <span class="text-white font-medium mb-1">${normalizeOutcomeTitle(outcome.title)}</span>
                    <span class="text-xl font-semibold ${multiColor.text}">${formatProbabilityDisplay(outcome.probability)}</span>
                  </button>
                `;
      }).join('')
    }
      ...
      return `
                      <div class="flex items-center justify-between">
                        ...
                        <span class="font-medium text-sm text-slate-300">${formatPercentDisplay(outcome.probability)}</span>
                      </div>
                    `;
    }).join('')}
                </div>
