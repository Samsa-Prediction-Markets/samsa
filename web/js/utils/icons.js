...
const SVG_ICONS = {
  ...
  starFilled: `<svg class="w-full h-full" fill="${GOLD_COLOR}" stroke="${GOLD_COLOR}" stroke-width="1.5" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>`,

  // Warning / alert triangle
  warning: `<svg class="w-full h-full" style="color: ${GOLD_COLOR};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`,

  // Person / user / account
  person: `<svg class="w-full h-full" style="color: ${GOLD_COLOR};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`,

  // Trending up (for news related markets)
  trending: `<svg class="w-full h-full" style="color: ${GOLD_COLOR};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>`,

  // Scale / balance (outline, simpler version for type badge)
  balance: `<svg class="w-full h-full" style="color: ${GOLD_COLOR};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M12 3v18m-6-9H3l3-6 3 6m12 0h-3l3-6 3 6"/><path d="M5 21h14"/></svg>`,

  // Bar chart (simpler for type badge)
  barchart: `<svg class="w-full h-full" style="color: ${GOLD_COLOR};" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><rect x="18" y="3" width="3" height="18"/><rect x="10.5" y="8" width="3" height="13"/><rect x="3" y="13" width="3" height="8"/></svg>`,
...
/**
 * Get star icon HTML for watchlist buttons
 * @param {boolean} isWatchlisted - Whether the item is watchlisted
 * @param {string} sizeClass - Tailwind size class (default 'w-5 h-5')
 * @returns {string} SVG HTML string
 */
function getStarHTML(isWatchlisted, sizeClass = 'w-5 h-5') {
  const svgKey = isWatchlisted ? 'starFilled' : 'star';
  let svg = SVG_ICONS[svgKey];
  svg = svg.replace('class="w-full h-full"', `class="${sizeClass}"`);
  if (!isWatchlisted) {
    svg = svg.replace(`style="color: ${GOLD_COLOR};"`, 'style="color: #64748b;"');
  }
  return svg;
}

/**
 * Get an SVG icon by its direct key (not name-mapped)
 * @param {string} key - Direct SVG_ICONS key
 * @param {string} sizeClass - Tailwind size class
 * @param {string} color - Override color (CSS color value)
 * @returns {string} SVG HTML string
 */
function getIconByKey(key, sizeClass = 'w-5 h-5', color = null) {
  let svg = SVG_ICONS[key] || SVG_ICONS.default;
  svg = svg.replace('class="w-full h-full"', `class="${sizeClass}"`);
  if (color) {
    svg = svg.replace(`style="color: ${GOLD_COLOR};"`, `style="color: ${color};"`);
    svg = svg.replace(`fill="${GOLD_COLOR}"`, `fill="${color}"`);
    svg = svg.replace(`stroke="${GOLD_COLOR}"`, `stroke="${color}"`);
  }
  return svg;
}
