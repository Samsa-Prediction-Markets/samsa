...
document.addEventListener('DOMContentLoaded', function () {
  // Show dashboard immediately so the user never sees a blank screen
  showDashboard();
  ...
  (async function loadData() {
    try { Auth.init(); } catch (e) { console.log('Auth init skipped:', e.message); }
    try { await API.init(); } catch (e) { console.log('API init skipped:', e.message); }
    try {
      if (typeof initializeWallet === 'function') {
        await initializeWallet();
      }
    } catch (e) { console.log('Wallet init skipped:', e.message); }
    try { await renderMarkets(); } catch (e) { console.log('Markets load error:', e.message); }
